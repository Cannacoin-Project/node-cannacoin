...
`npm install git://github.com/cannacoin-project/node-cannacoin-master/express.git`

## Examples

Some code examples follow below, but for more complete examples, see [these snippets](https://github.com/cannacoin-project/cannacoin-code-snippets), or this [wallet app](comingsoon...) which was created to to test this module.

```js
var cannacoin = require('node-cannacoin')()

cannacoin.auth('myusername', 'mypassword')

cannacoin.getDifficulty(function() {
...
```js
cannacoin.getNewAddress(function(err, address) {
...
```js
cannacoin.exec('getNewAddress')

cannacoin.exec('getbalance', function(err, balance) {
...
var cannacoin = require('cannacoin')({
    user:'user'
})
...
By default `cannacoind` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `cannacoind` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `cannacoin.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/cannacoind.crt
    rpcsslprivatekeyfile=/etc/ssl/private/cannacoind.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `cannacoind.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-cannacoin is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('cannacoind.crt')
...
nodeunit test/test-node-cannacoin.js
```
